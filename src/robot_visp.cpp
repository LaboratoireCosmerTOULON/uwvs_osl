							...
							)
{
    ...
    if(!init)
    {
 		...
 		// camera parameters
    	double px = 600; // pixel width
    	double py = 600; // pixel high
        ...
        for (unsigned int i = 0 ; i < 4 ; i++) {
        /* Track the feature parameters in the camera frame and     
           compute the projection of these parameters in the 
           image plane*/
            point[i].track(cdMo);
		...
		utemp = bRect.x; 				vtemp = bRect.y;
        vpPixelMeterConversion::convertPoint (cam, utemp, vtemp, xtemp, ytemp);
        pc[0].buildFrom(xtemp,ytemp,1);

	    utemp = bRect.x + bRect.width; 	vtemp = bRect.y;
        vpPixelMeterConversion::convertPoint (cam, utemp, vtemp, xtemp, ytemp);
        pc[1].buildFrom(xtemp,ytemp,1);
        
		utemp = bRect.x + bRect.width; 	vtemp = bRect.y + bRect.height;
        vpPixelMeterConversion::convertPoint (cam, utemp, vtemp, xtemp, ytemp);
        pc[2].buildFrom(xtemp,ytemp,1);
	    
		utemp = bRect.x; 				vtemp = bRect.y + bRect.height;
        vpPixelMeterConversion::convertPoint (cam, utemp, vtemp, xtemp, ytemp);
...
void motionControlVisp(double xd, double yd, double zd, double rolld, double pitchd, double yawd, double lambda,cv::Rect bRect)
{
	...
	vpPoseVector rpc(0,0,0,0,0,0.5*M_PI);
...
//COMMENTstd::cerr << counter <<" " << v.transpose() << std::endl;
std::cerr << std::endl << "teste..." << vr<< std::endl;  
std::cerr << "error"<<task.getError().transpose() <<std::endl;   
    ...
    ts.twist.angular.x = 0;//vr[3];
    ts.twist.angular.y = 0;//vr[4];
    ts.twist.angular.z = 0;//vr[5];
	pub.publish(ts);
}
void imageCallBack(const sensor_msgs::ImageConstPtr& msg) {
	...
	motionControlVisp(0, 0, 3, 0, 0, 0, 0.05, bRect);
